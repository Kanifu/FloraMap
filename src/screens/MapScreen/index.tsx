...
const PlantMenu = ({ plant, onClose, onMove, onResize, onDelete, onChangeColor, onSaveNote, onChangePlantedDate }: PlantMenuProps): React.JSX.Element | null => {
  ...
  const menuStyles = useMemo(() => makeMenuStyles(theme), [theme]);
...
const MapScreen = (): React.JSX.Element => {
  ...
  const handleScan = async (fromGallery = false) => {
    if (fromGallery) {
      ...
      if (status !== 'granted') {
        Alert.alert('Toestemming nodig', 'Geef toegang tot je fotobibliotheek om een plant te scannen.');
        return;
      }
    } else {
      const { status } = await ImagePicker.requestCameraPermissionsAsync();
      if (status !== 'granted') {
        Alert.alert('Toestemming nodig', 'Geef toegang tot de camera om een plant te scannen.');
        return;
      }
    }
  ...
  const styles = useMemo(() => StyleSheet.create({
    ...
    emptyContainer:        { flex: 1, alignItems: 'center', justifyContent: 'center', backgroundColor: theme.card, padding: 32, gap: 14 },
    ...
    // Dashboard bar — intentional dark brand colors, kept hardcoded
    dashBar: {
  ...
  const corrStyles = useMemo(() => StyleSheet.create({
    ...
    lowConf: { fontSize: 12, color: '#e85d04', marginBottom: 8, backgroundColor: theme.warningLight, padding: 8, borderRadius: 8 },

...
import { MaintenanceTask, MaintenanceTaskType, Plant, GardenTask, BADGE_DEFINITIONS } from '@/models';
...
const MaintenanceScreen = (): React.JSX.Element => {
  ...
  // Show toast for the badge that was *just* unlocked (driven by the store's
  // recentUnlockId), instead of guessing from array position/order.
  useEffect(() => {
    if (!recentUnlockId) return;
    const newBadge = BADGE_DEFINITIONS.find((b) => b.id === recentUnlockId);
    if (newBadge) {
